...
import StaggerText from "@/Molecules/stagger-text";
const UmiTemplate = () => {
              ...
              >
                {currentWordIndex >= wordIndex && (
                  <div
                    key={wordIndex}
                    style={{ color: "yellow" }}
                    className="absolute top-0 left-0 text flex flex-row items-center justify-start"
                  >
                    <StaggerText
                      height={wordDimention.height + 4}
                      text={word.word}
                      width={wordDimention.width + 8}
                      fontSize={`${size[0]}`}
                      color={color}
                    />
                  </div>
